...
public class MapDisplayFragment extends Fragment implements MapEventsReceiver, IMyLocationConsumer {
    ...
    private void plotRoutes() {
        ...
        if(s != null)
        {
            for(Route r : s.getRoutes())
            {
                ...
                int routeColor = busRouteLegendOverlay.getColor(r.getNumber());

                for(RoutePattern routePattern : r.getPatterns())
                {
                    List<GeoPoint> geoPoints = convertToListOfGeoPoints(routePattern.getPath());
                    int pointSize = geoPoints.size();
                    int indexOfLast = pointSize - 1;

                    LatLon src = new LatLon(geoPoints.get(0).getLatitude(),
                            geoPoints.get(0).getLongitude());
                    LatLon dst = new LatLon(geoPoints.get(indexOfLast).getLatitude(),
                            geoPoints.get(indexOfLast).getLongitude());
    ...
    private List<GeoPoint> convertToListOfGeoPoints(List<LatLon> list)
    {
        List<GeoPoint> geoPoints = new ArrayList<>();
        for(LatLon pt : list)
        {
            GeoPoint g = new GeoPoint(pt.getLatitude(), pt.getLongitude());
            geoPoints.add(g);
        }
...
//                + southeast.getLatitude() + " " + southeast.getLongitude());
    ...
    private void markStops() {
        ...
        updateOverlays();
        parseStops();
        Iterator<Stop> iterator = StopManager.getInstance().iterator();
        while (iterator.hasNext()) {
            Stop s = iterator.next();
            if (Geometry.rectangleContainsPoint(northWest, southEast, s.getLocn())) {
                Marker mark = new Marker(mapView);
                mark.setIcon(stopIconDrawable);
                setMarker(s, mark);
                mark.setInfoWindow(stopInfoWindow);
                mark.setTitle("StopNo : " + s.getNumber() + "\nStopName : " + s.getName());
                mark.setRelatedObject(s);
                GeoPoint geoPoint = new GeoPoint(s.getLocn().getLatitude(), s.getLocn().getLongitude());
                mark.setPosition(geoPoint);
                stopClusterer.add(mark);
            }
        }
    ...
    private void updateMarkerOfNearest(Stop nearest) {
        ...
        if(nearest != null) {
            List<Marker> markers;
            markers = stopClusterer.getItems();
            for (int i = 0; i < markers.size(); i++) {
                if (stopClusterer.getItem(i).getRelatedObject().equals(nearest)) {
                    stopClusterer.getItem(i).setIcon(closestStopIconDrawable);
    ...
    private void handleLocationChange(Location location) {
        Drawable closestStopIconDrawable = getResources().getDrawable(R.drawable.closest_stop_icon);
        lastKnownFromInstanceState = new Location(location);
        ...
        BusesAreUs.activity.onLocationChanged(nearest, latLon);
